...
parser.add_argument('--lrx', type=int,   default=1,          help='step size to perturbate the image')
parser.add_argument('--eps', type=int,   default=10,         help='maximal allowed perturbations')
...
if isinstance(dset.train_data,np.ndarray):
    x = torch.from_numpy(dset.train_data)  # only wrapper, change of x is automatically inherited by value of train_data
    y = torch.LongTensor(dset.train_labels)
else:
    x,y = dset.train_data,dset.train_labels

if x.dim() == 3:
    x  = x.unsqueeze(1)
x0 = x.clone().float()/255.0
...
for epoch in range(1,opt.nepochs+1):
    ...
    # update adversrial data set
    x = attacks.batch_one_step_attack(model,criterion,x,y,x0,lr=opt.lrx/255.0,eps=opt.eps/255.0,batch_size=128)
    ...
    torch.save(model.state_dict(),'pretrains/save.pkl')
    print('Best Model: %.2f%%\t %.2f%%'%(best_acc1,best_acc5))

torch.save(model.state_dict(),'pretrains/adversarial/%s-%s-%.2f.pkl'%(opt.arch,opt.dataset,best_acc1))
